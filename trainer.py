...
class train_callback(pl.Callback):
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if args.lr_final == args.lr_init or args.epoch_count == 0:
            ...
            # 线性或指数衰减
            if args.lr_final == 0 or args.lr_init == 0:  # 线性衰减
        ...
        if trainer.global_step == 0:
            if trainer.is_global_zero:  # 仅在主进程记录日志
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if trainer.is_global_zero:  # 记录训练指标
            t_now = time.time_ns()
            kt_s = 0
            try:
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        args = self.args
        to_save_dict = {}
        if (trainer.is_global_zero) or ('deepspeed_stage_3' in args.strategy):  # 保存检查点
        ...
        if trainer.is_global_zero:  # 记录epoch日志
            trainer.my_log.write(f"{args.epoch_begin + trainer.current_epoch} {trainer.my_epoch_loss:.6f} {math.exp(trainer.my_epoch_loss):.4f} {trainer.my_lr:.8f} {datetime.datetime.now()} {trainer.current_epoch}\n")
            trainer.my_log.flush()

            trainer.my_loss_sum = 0
            trainer.my_loss_count = 0
            if (args.epoch_begin + trainer.current_epoch) >= args.my_exit:  # 达到退出条件
                exit(0)
...
def generate_init_weight(model, init_weight_name):
    ...
    if model.args.my_pile_stage == 1:  # 阶段1的特殊处理
    ...
    if model.args.my_pile_stage == 1:  # 阶段1完成后退出
